...
import { ArrowLeft, Tag, Globe, Building, Users, Sparkles, AlertCircle, Zap, Shield, Calendar } from 'lucide-react';
...
export default function EventPostForm({ onSubmit, onCancel, events }: EventPostFormProps) {
  const [formData, setFormData] = useState<EventPostForm>({
    title: '',
    eventId: '',
    situation: '',
    innovation: '',
  ...
  const handleInputChange = (field: keyof EventPostForm, value: string) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.eventId) newErrors.eventId = 'イベントの選択は必須です';
  ...
  const getEventStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'upcoming': return 'text-sky-blue';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">イベント事例を投稿</h1>
          <p className="text-gray-600 mt-1">イベントでの成功体験を仲間と共有しましょう</p>
        </div>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div className="flex items-start space-x-3">
          ...
          <div>
            <h3 className="font-medium text-sky-blue mb-1">イベント事例として投稿</h3>
            <p className="text-sm text-blue-700">
              この投稿は選択したイベントページにも自動で反映され、
              他の店舗スタッフがイベント準備時に参考にできます。
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-900 mb-2">
              対象イベント <span className="text-vivid-red">*</span>
            </label>
            <select
            ...
            {selectedEvent && (
              <div className="mt-2 flex items-center space-x-2">
                ...
                <span className="text-sm font-medium text-gray-900">{selectedEvent.name}</span>
                <span className={`text-sm ${getEventStatusColor(selectedEvent.status)}`}>
          ...
          <div>
            ...
            <input
              ...
              placeholder="例：呪術廻戦フェアでのアニメファン接客成功事例"
              className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-sky-blue focus:border-transparent ${
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="例：20代男性、呪術廻戦フェアでフィギュア購入、イベント初日来店"
              rows={3}
              className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-sky-blue focus:border-transparent ${
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="例：お客様の好きなキャラクターについて会話を始め、そのキャラクターグッズでポイントが貯まることを具体例で説明"
              rows={4}
              className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-sky-blue focus:border-transparent ${
